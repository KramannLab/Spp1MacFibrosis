...
setwd("c:/Users/Konra/Dropbox/Kramann Arbeitsgruppe/2. CXCl4/In Vivo/snRNA-Data/for_sikander_konrad/ecm/Fibro_Revision/")
...
# Annotation
Idents(Samples.combined) <- "leiden0.3"
new.cluster.names <- c("Fibro 1", "DCT","PT", "Endo", "TAL", "Fibro 2", "mtGene Cluster", "MC", "DTL", "IC", "Meg3 Fib", "Podo")
...
Samples.combined <- subset(Samples.combined, idents=c("Fibro 1", "Fibro 2", "MC", "Meg3 Fib"))
...
top3 <-unique(top3)
DotPlot(Samples.combined, features = top3,dot.scale = 4)  + scale_y_discrete(limits=c("Meg3 Fib","MC", "Fibro 2", "Fibro 1")) + theme(
          axis.title = element_blank(),
          text = element_text(size=8, family = "Arial"),
          axis.text.x = element_text(size=8, family = "Arial", angle=45, hjust=1),
          axis.text.y = element_text(size=8, family = "Arial"),
...
my_levels <- c("Fibro 1 Sham", "Fibro 1 IRI","Fibro 2 Sham", "Fibro 2 IRI", "MC Sham", "MC IRI", "Meg3 Fib Sham", "Meg3 Fib IRI")
...
ggsave(filename = paste0("ECM scoring output/",sample,"_",gset,"_vln.svg"), width=3.5 , height = 2.2)
...
print(t.test(Naba.df[which((Naba.df$Annotation=="MC")&(Naba.df$condition=="WT IRI")),3],Naba.df[which((Naba.df$Annotation=="MC")&(Naba.df$condition=="Cxcl4-/- IRI")),3], alternative = "two.sided"))
print(paste0("t-test for ", gset, " Fibroblast 4 IRI WT vs KO"))
print(t.test(Naba.df[which((Naba.df$Annotation=="Meg3 Fib")&(Naba.df$condition=="WT IRI")),3],Naba.df[which((Naba.df$Annotation=="Meg3 Fib")&(Naba.df$condition=="Cxcl4-/- IRI")),3], alternative = "two.sided"))
...
#Plot Dapk2
# Step 5: Violin Plots for "Dapk2"
Fib_colors<- c("#1D6996","#0F8554","#E17C05","#6F4070")
VlnPlot(Samples.combined, features = "Dapk2",  group.by="Annotation", cols = Fib_colors, pt.size = 0) + NoLegend()+ 
    geom_boxplot(width=0.1,position = position_dodge(1),outlier.shape = NA,coef=0)+theme(
      axis.title = element_blank(),
      text = element_text(size=8, family = "Arial"),
      axis.text.x = element_text(size=8, family = "Arial", angle=45, hjust=1),
      ...
      axis.line = element_line(size = 0.2), 
      axis.ticks = element_line(size = 0.2))
ggsave(filename = paste(sample,"Vln_Dapk2.jpeg"), width=1.5 , height = 1.7)
ggsave(filename = paste(sample,"Vln_Dapk2.svg"), width=1.5 , height = 1.7)
...
# Run Progeny
sc <- progeny(sc, scale=FALSE, organism="Mouse", top=500, perm=1, 
              return_assay = TRUE, assay_name = "originalexp")
sc <- Seurat::ScaleData(sc, assay = "progeny")

#Create dataframe with annotated clusters and progeny scores
progeny_scores_df <- as.data.frame(t(GetAssayData(sc, slot = "scale.data", 
                                                  assay = "progeny"))) %>%
...
progeny_hmap = pheatmap(summarized_progeny_scores_df[,-1],fontsize=14, 
                        fontsize_row = 10, 
                        color=myColor, breaks = progenyBreaks, cluster_rows = F,
                        main = "PROGENy (500)", angle_col = 45,
                        treeheight_col = 0,  border_color = NA,
                        filename = paste0(sample, "05_Progeny.pdf"))
